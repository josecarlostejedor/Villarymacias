...
export default function App() {
  ...
  const handleBalizaSubmit = (code: string) => {
    ...
    setBalizaTimestamps(prev => [...prev, now]);
  ...
  const calculateResults = () => {
    ...
    // Usamos el último timestamp registrado como fin de carrera si existe
    const endTime = balizaTimestamps.length > 0 ? balizaTimestamps[balizaTimestamps.length - 1] : Date.now();
    const totalTime = endTime - (startTime || 0);
    
    const results: BalizaResult[] = selectedRoute.balizas.map((b, idx) => {
      // El tiempo de la baliza actual está en idx + 1 (el index 0 es el inicio)
      const currentTime = balizaTimestamps[idx + 1];
      const previousTime = balizaTimestamps[idx];
      
      const pTime = (currentTime !== undefined && previousTime !== undefined) 
  ...
  const generatePDF = async () => {
    ...
    console.log("Iniciando generación de PDF con tiempos parciales...");
