...
async function startServer() {
  ...
  app.post("/api/results", async (req, res) => {
    ...
    try {
      ...
      // Calculate score for Google Sheets - Robust type handling
      ...
      if (googleSheetsUrl && googleSheetsUrl.startsWith("http")) {
        try {
          ...
          const maskedUrl = `${googleSheetsUrl.substring(0, 15)}...${googleSheetsUrl.substring(googleSheetsUrl.length - 4)}`;
          console.log(`[SHEETS] Intentando envío a Google Sheets (${maskedUrl})...`);
          ...
          try {
            // Usamos text/plain para evitar problemas de CORS/Preflight en Google Apps Script
            // El script de Google lo recibirá igual en e.postData.contents
            ...
            console.log(`[SHEETS] Respuesta recibida (${sheetResponse.status}):`, resultText);
          } catch (fetchError: any) {
            clearTimeout(timeoutId);
            if (fetchError.name === 'AbortError') {
              console.error("[SHEETS] Error: Tiempo de espera agotado (10s).");
            } else {
              throw fetchError;
            }
          }
          ...
          console.error("[SHEETS] Error en el proceso de envío:", sheetError.message);
        }
      } else {
        console.warn("[SHEETS] No se enviará a Google Sheets: URL no configurada o inválida en Vercel.");
      }
